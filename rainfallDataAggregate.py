...
if __name__ == "__main__":
    try:
        ...
        logging.basicConfig(level=logging.DEBUG,
                            ...
                            filename=os.path.join(path, 'rainfall-info.log'),
                            filemode='w')
        for item in os.listdir(path):
            ...
            if(os.path.isdir(itempath)
                ...
                for i in range(BEGINCOL, ENDCOL + 1):
                    if( not d.get(sheet.cell(DATEROW, i).value, False)):
                        d[sheet.cell(DATEROW, i).value] = sheet.col_values(i)
        ...
        wbk.save(os.path.join(path, "rainfall.xls"))
    except Exception as e:
